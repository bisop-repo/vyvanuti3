...
data <- data %>%
  mutate(status = as.numeric(case_when(
    DeadByCovid == 1 ~ 2,  # Event of interest
    DeadByOther == 1 ~ 1,    # Competing event
    TRUE ~ 0               # Censored
  )))

#data$status <- factor(data$status, levels = c(0, 1, 2))  



f <- as.formula(Surv(T1, T2, Dead) ~ VaccStatus  + DCCI + Sex)
