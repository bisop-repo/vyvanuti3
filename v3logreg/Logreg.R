...
m1_logreg <- glm(Seriouscovidproxy ~ VariantOfInf + InfPrior + VaccStatus + AgeGr + Sex,
                 family = binomial(link = "logit"), data = data)
summary(m1_logreg)

m2_logreg <- glm(LongCovid ~ Seriouscovidproxy + VariantOfInf + InfPrior + VaccStatus + AgeGr + Sex,
                 family = binomial(link = "logit"), data = data)
summary(m1_logreg)
...
ggsave("serious_logreg_forest_plot.jpg", units = "px", width = 3840, height = 3840)

forest_model(m2_logreg)

# forest_model(m1_logreg, covariates = "VariantComp")
ggsave("long_logreg_forest_plot.jpg", units = "px", width = 3840, height = 3840)
